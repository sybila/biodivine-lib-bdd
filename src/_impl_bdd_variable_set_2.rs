...
impl<T: Clone + Eq + Hash + Debug> BddVariableSet2<T> {
    /// Create a new `BddVariableSet` with anonymous variables $(x_0, \ldots, x_n)$ where $n$ is
    /// the `num_vars` parameter.
    pub fn new_anonymous(num_vars: u16) -> BddVariableSet2<String> {
        ...
        BddVariableSet2 {
            num_vars,
            variables: (0..num_vars).map(|i| format!("x_{}", i)).collect(),
    ...
    pub fn new(vars: &[T]) -> BddVariableSet2<T> {
        BddVariableSet2 {
            num_vars: u16::try_from(vars.len()).unwrap(),
            variables: Vec::from_iter(vars.iter().cloned()),
            var_index_mapping: vars
                .iter()
                .cloned()
                .enumerate()
                .map(|(a, b)| (b, u16::try_from(a).unwrap()))
                .collect(),
        }
    }
    ...
    pub fn find_variable(&self, variable: &T) -> Option<BddVariable> {
        self.var_index_mapping
            .get(variable)
            .cloned()
            .map(BddVariable)
    }
    ...
    pub fn get_variable(&self, variable: BddVariable) -> &T {
    ...
    pub fn mk_var_by_ref(&self, var: &T) -> Bdd {
        self.find_variable(var)
    ...
    ///
    /// *Panics:* `var` must be a name of a valid variable in this set.
    pub fn mk_not_var_by_ref(&self, var: &T) -> Bdd {
        self.find_variable(var)
    ...
    ///     [BddVariableSet].
    ///
    pub fn transfer_from(&self, bdd: &Bdd, ctx: &BddVariableSet2<T>) -> Option<Bdd> {
        ...
        for var in &old_support_set {
            let variable = ctx.get_variable(*var);
            let Some(id) = self.find_variable(variable) else {
...
impl FromIterator<String> for BddVariableSet2<String> {
    fn from_iter<T: IntoIterator<Item = String>>(iter: T) -> Self {
        // TODO: Double copy.
        let names = iter.into_iter().collect::<Vec<_>>();
        BddVariableSet2::new(&names)
    }
}

impl From<Vec<String>> for BddVariableSet2<String> {
    fn from(value: Vec<String>) -> Self {
        BddVariableSet2::from_iter(value)
    }
}

impl From<Vec<&str>> for BddVariableSet2<String> {
    fn from(value: Vec<&str>) -> Self {
        BddVariableSet2::from_iter(value.iter().map(|it| it.to_string()))
    }
}
...
mod tests {
    use super::*;
    use crate::_test_util::mk_5_variable_set_2;
    use num_bigint::BigInt;

    #[test]
    fn bdd_universe_anonymous() {
        let universe = BddVariableSet2::<String>::new_anonymous(5);
        assert_eq!(
            Some(BddVariable(0)),
            universe.find_variable(&"x_0".to_string())
        );
        assert_eq!(
            Some(BddVariable(1)),
            universe.find_variable(&"x_1".to_string())
        );
        assert_eq!(
            Some(BddVariable(2)),
            universe.find_variable(&"x_2".to_string())
        );
        assert_eq!(
            Some(BddVariable(3)),
            universe.find_variable(&"x_3".to_string())
        );
        assert_eq!(
            Some(BddVariable(4)),
            universe.find_variable(&"x_4".to_string())
        );
    }

    #[test]
    fn bdd_universe_mk_const() {
        let variables = mk_5_variable_set_2();
    ...
    fn bdd_universe_mk_var_invalid_id() {
        mk_5_variable_set_2().mk_var(BddVariable(6));
    }
    ...
    fn bdd_universe_mk_not_var_invalid_id() {
        mk_5_variable_set_2().mk_not_var(BddVariable(6));
    }

    #[test]
    #[should_panic]
    fn bdd_universe_mk_var_by_name_invalid_ref() {
        mk_5_variable_set_2().mk_var_by_ref(&"abc".to_string());
    }

    #[test]
    #[should_panic]
    fn bdd_universe_mk_not_var_by_name_invalid_ref() {
        mk_5_variable_set_2().mk_not_var_by_ref(&"abc".to_string());
    }
    ...
    fn bdd_mk_sat_k() {
        ...
        let vars = BddVariableSet2::<String>::new_anonymous(5);
    ...
    #[test]
    fn bdd_transfer() {
        let ctx_1 = BddVariableSet2::<String>::new(&["a", "b", "x", "c", "y"]);
        let ctx_2 = BddVariableSet2::<String>::new(&["a", "x", "b", "z", "c"]);
    ...
    fn bdd_variable_enum() {
        ...
        let ctx = BddVariableSet2::new(&[
            Variable::TypeA(0),
            Variable::TypeB("Hello".to_string()),
            Variable::TypeA(5),
            Variable::TypeB("World".to_string()),
        ]);

        assert_eq!(
            BddVariable::from_index(0),
            ctx.find_variable(&Variable::TypeA(0)).unwrap()
        );
        assert_eq!(
            BddVariable::from_index(1),
            ctx.find_variable(&Variable::TypeB("Hello".to_string()))
                .unwrap()
        );
        assert_eq!(
            BddVariable::from_index(2),
            ctx.find_variable(&Variable::TypeA(5)).unwrap()
        );
        assert_eq!(
            BddVariable::from_index(3),
            ctx.find_variable(&Variable::TypeB("World".to_string()))
                .unwrap()
        );
    }
}
