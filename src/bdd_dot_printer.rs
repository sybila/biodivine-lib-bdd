...
pub fn bdd_as_dot(
    ...
    */
    ...
    if !zero_pruned {
        output.write_all(
            b"0 [shape=box, label=\"0\", style=filled, shape=box, height=0.3, width=0.3];\n",
        )?;
    }
    output.write_all(
        b"1 [shape=box, label=\"1\", style=filled, shape=box, height=0.3, width=0.3];\n",
    )?;
    // decision nodes
...
pub fn bdd_as_dot_string(bdd: &Bdd, var_names: &Vec<String>, zero_pruned: bool) -> String {
    ...
    bdd_as_dot(&mut buffer, bdd, var_names, zero_pruned).expect("Cannot write BDD to .dot string.");
